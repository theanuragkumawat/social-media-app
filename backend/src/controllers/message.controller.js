...
const getUsersForSidebar = asyncHandler(async (req, res) => {
    ...
    res.json(new ApiResponse(200, "Chats fetched successfully", chats));
...
const markMessageAsSeen = asyncHandler(async (req,res) => {
    ...
    res.json(new ApiResponse(200, "Message marked as seen", message));
})

const sendMessage = asyncHandler(async (req, res) => {
    const senderId = new mongoose.Types.ObjectId(req.user?._id);
    const recipientId = new mongoose.Types.ObjectId(req.body.userId);
    ...
    res.json(new ApiResponse(201, "Message sent successfully", message));
})
